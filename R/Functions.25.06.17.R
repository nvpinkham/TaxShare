...
#' @export
tax.shared.plot <- function (otu.input, var, group1, group2,
                             tax.level = tax$family,
                             PresAbs = F,
                             log = T,
                             min.obs = 1,
...
#  res.all <- res.all[complete.comps]
  ...
  for (i in (1 : n)[complete.comps]) {
    ...
    shared.1 <- colorspace::mixcolor(0.2, colorspace::sRGB(t(col2rgb(var1.color1[p][1]))),
                                     colorspace::sRGB(t(col2rgb(var1.color2[p][1]))))
    shared.2 <- colorspace::mixcolor(0.5, colorspace::sRGB(t(col2rgb(var1.color1[p][1]))),
                                     colorspace::sRGB(t(col2rgb(var1.color2[p][1]))))
